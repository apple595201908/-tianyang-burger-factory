...
視覺 Scale 以短邊為基準並限制在合理範圍；Gameplay 速度、食品寬度與 205 基準間距使用同一個 Motion Scale，確保不同尺寸的操作時間與密度一致。後期 Rush Pair 會把單次生成目標縮短至正常間距的一半，但仍受最小輪廓空隙保護。
...
```text
outputs/Tianyang-Burger-Factory-HD-Xiaomi-Pad-Mini-v1.1.7.apk
```
...
- 左上／左中／左下皆為 FINISH。
- 右上／右中／右下皆為 RETURN。
- 回捲速度與前進速度一致，全部漢堡保持回捲前的正常／Rush 間距型態。
- 回捲期間玩家站右側完成品不可造成誤判。
- 旋轉後不 Reload、不交換左右操作。
...
- 正常漢堡間距至少大於完整 Action Window；40 分後偶發的 Rush Pair 是刻意縮短的後期挑戰，但仍禁止輪廓相黏。
...
## 16. V1.1.7 前期提速與 Rush Pair

正式遊戲起始速度由 128 提高為 145，約增加 13%；每分速度增量由 2 提高為 2.2，每 10 分仍額外增加 4，最高速度由 465 調整為 485。Tutorial 維持固定速度，不參與正式難度曲線；回捲依然直接使用當下正轉速度，因此兩個方向永遠等速。

40 分後解鎖 Rush Pair：下一顆漢堡可能只隔正常間距的一半生成。觸發率在 40 分為 5%，到 120 分平滑提升至最高 18%；每次觸發後至少強制兩個正常間距，禁止短間距連發。考慮 Cooldown 後，實際短間距占比約從 4.5% 提高至最高 13.2%。Rush 使用獨立的可重現 RNG，不會改變初次漏料與補料偷懶原有的隨機序列。

`Layout.rushGap` 使用 `max(normalGap × 0.5, burgerVisualWidth + clearance)`，所以 Xiaomi Pad Mini 會得到精確半間距，小型手機則可在必要時略微放寬，避免漢堡輪廓重疊。Resize／旋轉會依目前排定的是正常或 Rush 間距重新換算生成進度；整線退回仍把同一位移扣回生成格點，不會因短間距破壞產線順序。

自動測試會驗證速度曲線、解鎖分數、5%～18% 機率、兩個正常間距 Cooldown、Pad Mini 精確半間距、最小視覺空隙，以及旋轉時未完成生成進度的比例換算。
