...
type BurgerAssembly = "base" | "open" | "closed";
type Action = "FINISH" | "RETURN";
type FailureReason = "WRONG_FINISH" | "WRONG_RETURN" | "MISSED_DEADLINE";
type StationRole = "base" | "fill" | "player" | "inspector";
type StationAction = "place" | "skip" | "cap" | "push" | "stamp";

const ACTION_CLASSES = ["acting-place", "acting-skip", "acting-cap", "acting-push", "acting-stamp"] as const;
const ACTION_MS = {
  place: 500,
  skip: 400,
  cap: 440,
...
class CharacterActionDirector {
  ...
  public play(station: StationRole, action: StationAction, durationMs: number = ACTION_MS[action]): void {
...
interface BurgerVisualContract {
  assembly: BurgerAssembly;
  needsReturn: boolean;
  label: string;
}

interface SpawnAdvance {
  progress: number;
  overflow: number | null;
}

interface SpawnGapDecision {
  gap: number;
  cooldown: number;
  rush: boolean;
}

interface StationRatios {
  base: number;
  fill: number;
  player: number;
  inspector: number;
...
// This table mirrors the Web V1 FoodItem redraw contract. Gameplay state owns
// the visual layers; CSS state names never decide independently which pieces
// of the burger exist.
const BURGER_VISUAL_CONTRACT: Readonly<Record<FoodVisualState, BurgerVisualContract>> = {
...
const FOOD_STATE_TRANSITIONS: Readonly<Record<FoodVisualState, readonly FoodVisualState[]>> = {
  spawned: ["base-only"],
...
function visualContractForFood(food: Pick<Food, "state" | "ingredientProcessed">): BurgerVisualContract {
...
function shouldShowTutorialMissingGuide(
  food: Pick<Food, "state" | "ingredientProcessed" | "tutorial">,
): boolean {
  return (
    food.tutorial &&
...
// Matches the Web V1 SpawnManager distance lattice. Keeping overflow is what
// makes the physical distance between foods independent of the frame rate.
function advanceSpawnLattice(progress: number, distance: number, gap: number): SpawnAdvance {
  const next = progress + Math.max(0, distance);
  if (next < gap) return { progress: next, overflow: null };
  const overflow = next - gap;
  return { progress: overflow, overflow };
}

function rewindSpawnLattice(progress: number, distance: number): number {
  return progress - Math.max(0, distance);
}

function rescaleSpawnLattice(progress: number, previousGap: number, nextGap: number): number {
  return progress * (nextGap / Math.max(1, previousGap));
}

function conveyorSpeedForScore(score: number): number {
  const safeScore = Math.max(0, score);
  const uncapped =
    START_SPEED + safeScore * SCORE_SPEED_STEP + Math.floor(safeScore / 10) * TEN_SCORE_BONUS;
  return Math.min(Math.round(uncapped * 10) / 10, MAX_SPEED);
}

function rushPairRateForScore(score: number): number {
  if (score < RUSH_START_SCORE) return 0;
  const progress = clamp(
    (score - RUSH_START_SCORE) / Math.max(1, RUSH_FULL_SCORE - RUSH_START_SCORE),
    0,
    1,
  );
...
function stationRatiosForOrientation(portrait: boolean): StationRatios {
...
function failureReactionDelay(prefersReducedMotion: boolean): number {
...
function isXiaomiPadMiniDisplay(width: number, height: number): boolean {
  const dpr = Math.max(1, window.devicePixelRatio || 1);
  const profileOverride = new URLSearchParams(window.location.search).get("profile") === "xiaomi-pad-mini";
  const screenCandidates: Array<[number, number]> = [
  ...
  const resolutionMatch = screenCandidates.some(
    ([longSide, shortSide]) =>
      near(longSide, PAD_MINI_LONG_PIXELS, 180) &&
...
class VectorBurgerFactory {
  ...
  private readonly renderNote = required<HTMLElement>("#render-note");
  private readonly store = new SaveStore();
  private readonly random = new RandomSource();
  private readonly rushRandom = new RandomSource(0x9e3779b9);
  private readonly audio = new GameAudio(this.store.value.mute);
  private readonly actions = new CharacterActionDirector();
  ...
  private nextSpawnGap = BASE_GAP;
  private nextSpawnIsRush = false;
  private rushCooldownSpawns = 0;
  ...
  private placeGuide(selector: string, x: number, y: number): void {
    required<SVGGElement>(selector).setAttribute("transform", `translate(${x} ${y}) scale(${this.layout.scale * 0.78})`);
  }
  ...
  private reflowFoods(previous: Layout, next: Layout): void {
    ...
    this.spawnDistance = rescaleSpawnLattice(
      this.spawnDistance,
      this.nextSpawnGap,
      nextTargetGap,
    );
  ...
  private createFood(state: FoodVisualState, x: number, tutorial = false): Food {
    ...
    ingredients.append(
      svgNode("path", "lettuce", { d: "M-49 8Q-37-3-24 8Q-10-4 2 8Q17-4 30 8Q41-2 49 8L44 18H-44Z" }),
      svgNode("rect", "patty", { x: -45, y: -2, width: 90, height: 20, rx: 9 }),
      svgNode("path", "cheese", { d: "M-44-8H44V4L28 12 13 4-4 12-20 4-35 10-44 3Z" }),
      svgNode("rect", "tomato", { x: -39, y: -16, width: 78, height: 11, rx: 5 }),
    );
    const sesame = svgNode("g", "sesame", {});
    sesame.append(
      svgNode("ellipse", "", { cx: -23, cy: -34, rx: 4, ry: 2, transform: "rotate(-24 -23 -34)" }),
      svgNode("ellipse", "", { cx: 0, cy: -45, rx: 4, ry: 2 }),
      svgNode("ellipse", "", { cx: 24, cy: -31, rx: 4, ry: 2, transform: "rotate(22 24 -31)" }),
      svgNode("ellipse", "", { cx: -6, cy: -23, rx: 4, ry: 2, transform: "rotate(18 -6 -23)" }),
    );
    visual.append(
      svgNode("ellipse", "food-shadow", { cx: 0, cy: 34, rx: 51, ry: 12 }),
      svgNode("rect", "bottom-bun", { x: -47, y: 15, width: 94, height: 26, rx: 12 }),
      ingredients,
      svgNode("path", "top-bun", { d: "M-47-13Q-38-58 0-60Q38-58 47-13Q30-5 0-5Q-30-5-47-13Z" }),
      sesame,
      svgNode("circle", "missing-ring", { cy: -5, r: 58 }),
    );
  ...
  private renderFood(food: Food): void {
    ...
    food.element.classList.toggle(
      "rework-success",
      food.state === "rework" && food.reworkWillSkip === false,
    );
  ...
  private transitionFood(food: Food, nextState: FoodVisualState): boolean {
  ...
  private finishFood(food: Food): void {
    if (!this.transitionFood(food, "capped")) return;
  ...
  private startReturn(food: Food): void {
    ...
    if (!this.transitionFood(food, "returning")) return;
  ...
  private updateReturn(deltaSeconds: number, now: number): void {
    ...
    if (this.returnPhase === "rewind") {
      ...
      this.foods.forEach((candidate) => {
        candidate.x -= shift;
        if (!candidate.hidden) this.renderFoodPosition(candidate);
      });
  ...
  private fail(reason: FailureReason): void {
    if (this.mode === "failing" || this.mode === "gameover") return;
    this.mode = "failing";
    this.activePointerId = null;
    this.actions.clear();
    this.app.classList.remove("returning", "reworking");
  ...
  private completeFailure(): void {
    if (this.mode !== "failing") return;
    this.mode = "gameover";
    this.gameOverOverlay.classList.remove("hidden");
  }

  private resetFailureReaction(): void {
  ...
  private updateLine(deltaSeconds: number, now: number): void {
    ...
    if (this.mode === "playing") {
      ...
      if (spawn.overflow !== null) {
        ...
        const decision = nextSpawnGapDecision(
          this.score,
          this.layout.gap,
          this.layout.rushGap,
          this.rushCooldownSpawns,
          this.rushRandom.next(),
        );
  ...
  private foodApproaching(stationX: number, match: (food: Food) => boolean): boolean {
    const reach = 82 * this.layout.scale;
    return this.foods.some(
      (food) => match(food) && food.x < stationX + 10 && stationX - food.x < reach && stationX - food.x > -4,
    );
  }

  private updateAnticipation(): void {
    const live = this.mode === "playing" || this.mode === "tutorial" || this.mode === "returning";
    this.actions.setAnticipating(
      "base",
      live && this.foodApproaching(this.layout.baseX, (food) => !food.baseProcessed),
    );
    this.actions.setAnticipating(
      "fill",
      live &&
        (this.foodApproaching(
          this.layout.fillX,
          (food) => food.baseProcessed && !food.ingredientProcessed,
        ) ||
          (this.mode === "returning" && this.returnPhase === "rework")),
    );
    this.actions.setAnticipating(
      "player",
      (this.mode === "playing" || this.mode === "tutorial") &&
        this.foodApproaching(
          this.layout.playerX,
          (food) =>
            food.ingredientProcessed &&
